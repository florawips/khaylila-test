...
@section('judul', 'Edit Data Jadwal')
@section('konten')
...
<div class="container mt-4">
    <div class="card form-card">
        ...
        <div class="card-body">
            <form action="{{ route('jadwal.update', $data['data']->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <option value="Senin" {{ old('hari', $data['data']->hari) == 'Senin' ? 'selected' : '' }}>Senin</option>
                    <option value="Selasa" {{ old('hari', $data['data']->hari) == 'Selasa' ? 'selected' : '' }}>Selasa</option>
                    <option value="Rabu" {{ old('hari', $data['data']->hari) == 'Rabu' ? 'selected' : '' }}>Rabu</option>
                    <option value="Kamis" {{ old('hari', $data['data']->hari) == 'Kamis' ? 'selected' : '' }}>Kamis</option>
                    <option value="Jumat" {{ old('hari', $data['data']->hari) == 'Jumat' ? 'selected' : '' }}>Jumat</option>
                    <option value="Sabtu" {{ old('hari', $data['data']->hari) == 'Sabtu' ? 'selected' : '' }}>Sabtu</option>
                    </select>
                    @error('hari')
                ...
                <div class="mb-3">
                    ...
                    <input
                        ...
                        class="form-control"
                        autocomplete="off"
                        value="{{ old('jam_ke', $data['data']->jam_ke) }}"
                    >
                    @error('jam_ke')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
